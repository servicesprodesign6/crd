...
class WhatsappStoreProductTransactionController extends AppBaseController
{
    ...
    public function downloadWpProductOrderPdf(WpOrder $wpProductOrder)
    {
        $wpProductOrder->load(['products.product.currency', 'wpStore']);
        $appLogo = getLogoUrl();
        $appName = getAppName();

        $pdf = \PDF::loadView('wp_product_transactions.wp_product_order_pdf', compact('wpProductOrder', 'appLogo', 'appName'));

        return $pdf->download('wp-product-order-' . $wpProductOrder->order_id . '.pdf');
    }
}