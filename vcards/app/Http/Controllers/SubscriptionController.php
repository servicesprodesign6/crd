...
class SubscriptionController extends AppBaseController
{
    ...
    public function choosePaymentType($planId, $context = null, $fromScreen = null)
    {
        ...
        $subscriptionsPricingPlan = Plan::with('planFeature')->findOrFail($planId);
    ...
    public function upgrade()
    {
        ...
        $plans = Plan::with(['currency', 'planFeature', 'planCustomFields', 'hasZeroPlan'])->whereStatus(Plan::IS_ACTIVE)->whereIsDefault(Plan::IS_DEACTIVE)->get();
    ...
    {
        $user = User::whereTenantId($request->tenant_id)
    ...
    public function userSubscribedPlanEdit(Request $request): JsonResponse
    {
        $subscription = Subscription::with([
            'plan:id,name,currency_id', 'tenant.user', 'tenant.organisationUser', 'plan.currency',
    ...
    public function downloadSubscriptionPdf($id)
    {
        $subscription = Subscription::with(['plan.currency', 'tenant.user', 'transactions'])->findOrFail($id);
        $appLogo = getLogoUrl();
        $appName = getAppName();
        $taxName = Setting::where('key', 'plan_tax_name')->value('value');

        $pdf = Pdf::loadView('subscription.subscription_pdf', compact('subscription', 'appLogo', 'appName', 'taxName'));

        return $pdf->download('subscription-' . $subscription->id . '.pdf');
    }
}
