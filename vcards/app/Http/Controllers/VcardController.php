...
class VcardController extends AppBaseController
{
    ...
    {
        if (! getLogInUser()->canCreateVcards()) {
        ...
        $orgUsers = User::role(Role::ROLE_ADMIN)
        ...
        return view('vcards.create', compact('partName', 'adminFavicon', 'orgUsers'));
    }
    ...
    public function show($alias, $id = null)
    {
        ...
        if ($vcard->status) {
            if (isVisitorLimitExceeded($vcard->id)) {
                return view('errors.visitor-limit-exceeded');
            }
        }
    ...
    {
        abort_if(! $vcard->isEditableBy(getLogInUser()), 403);
        ...
        $selectedOrgUsers = $vcard->assignedUser()->pluck('users.id')->toArray();
        ...
        return view('vcards.edit', compact('appointmentDetail', 'privacyPolicy', 'termCondition', 'iframes', 'managesection', 'instagramEmbed', 'banners', 'dynamicVcard', 'customLink','adminFavicon','services','appointmentServices','allWhatsappStores', 'selectedWhatsappStores', 'orgUsers', 'selectedOrgUsers'))->with($data);
    }
    ...
    public function getAvailableDays(Request $request): JsonResponse
    {
        try {
            ...
            return $this->sendError(__('messages.flash.something_went_wrong'));
        }
    }
    ...
    public function showBlog($alias, $id): \Illuminate\View\View
    {
        ...
        $blog = VcardBlog::with('vcard:id,template_id,name')->whereRelation('vcard', 'url_alias', '=', $alias)
            ...
            ->where('id', $id)
            ->firstOrFail();
        $vcardTemplate = vcard::find($blog->vcard_id);
        if ($vcardTemplate->status) {
            if (isVisitorLimitExceeded($vcardTemplate->id)) {
                return view('errors.visitor-limit-exceeded');
            }
        }
    ...
    public function showPrivacyPolicy($alias, $id)
    {
        ...
        setLocalLang(getLocalLanguage());
        if ($vacrdTemplate->status) {
            if (isVisitorLimitExceeded($vacrdTemplate->id)) {
                return view('errors.visitor-limit-exceeded');
            }
        }
    ...
    public function generateAiDescription(Request $request)
    {
        ...
        try {
            ...
            $description = $aiProvider === 'open_ai'
                ? $this->generateVcardDescriptionWithOpenAi($request->prompt, $userId)
                : $this->generateVcardDescriptionWithGemini($request->prompt, $userId);
        ...
        $response = Http::timeout(30)
            ...
            ->post('https://api.openai.com/v1/chat/completions', [
                ...
                'messages' => [
                    [
                        'role' => 'system',
                        'content' => $this->getVcardDescriptionPrompt(),
                    ],
                    [
                        'role' => 'user',
                        'content' => $prompt,
    ...
    private function generateVcardDescriptionWithGemini(string $prompt, int $userId): string
    {
            ...
            ->post('https://generativelanguage.googleapis.com/v1/models/' . $model . ':generateContent?key=' . $apiKey, [
                'contents' => [
                    [
                        'parts' => [
                            [
                                'text' => $this->getVcardDescriptionPrompt() . "\n\nUser prompt: " . $prompt,
                            ],
                        ],
                    ],
                ],
    ...
    public function aiGenerate(Request $request): JsonResponse
    {
        try {
            ...
            if (!$image->isValid()) {
                return $this->sendError(__('messages.vcard.invalid_image_uploaded'));
            }

            $userId = getLogInUser()->organisation_id ?: getLogInUserId();
            $aiProvider = getUserSettingValue('ai_vcard_modal', $userId);
            $openAiEnabled = getUserSettingValue('open_ai_enable', $userId);
            $geminiEnabled = getUserSettingValue('gemini_ai_enable', $userId);
            ...
            if (!empty($aiProvider)) {
                ...
                if ($openAiEnabled) {
                    ...
                    if(getLogInUser()->organisation_id == null){
                        ...
                        return $this->sendError(__('messages.organization.organization_user_ai_not_enabled_in_settings'));
                    }
                }
            }

            if ($aiProvider === 'open_ai') {
                $data = $this->generateWithOpenAi($image);
            } else {
                $data = $this->generateWithGemini($image);
            }

            if (!empty($data['invalid_card'])) {
                return $this->sendError(__('messages.vcard.uploaded_image_not_valid'));
            }

            $urlAlias = $this->generateUniqueAlias($data['company'] ?? 'ai-vcard');

            $input = [
                'name' => $data['company'] ?? 'AI vCard',
            ...
            return $this->sendResponse([
                'vcard_id' => $vcard->id,
                'redirect_url' => $url,
            ], 'AI vCard Generated Successfully');

        } catch (Exception $e) {

            Log::error('AI vCard Generation Error', [
                'message' => $e->getMessage()
            ]);
    ...
    private function generateWithGemini($image)
    {
        $userId = getLogInUser()->organisation_id ?: getLogInUserId();

        if (getUserSettingValue('gemini_enable', $userId) == 0) {
            throw new Exception(__('messages.vcard.gemini_not_enabled'));
        }
        $userApiKey = getUserSettingValue('gemini_api_key', $userId);
        $envApiKey = env('GEMINI_API_KEY');
    ...
    private function generateWithOpenAi($image)
    {
        $userId = getLogInUser()->organisation_id ?: getLogInUserId();

        if (getUserSettingValue('open_ai_enable', $userId) == 0) {
                    ...
                    [
                        ...
                        'content' => [
                            ...
                            [
                                'type' => 'image_url',
                                'image_url' => [
                                    'url' => 'data:'.$image->getMimeType().';base64,'.$base64Image
                                ]
                            ]
                        ]
                    ]
    ...
    private function generateUniqueAlias($base)
    {
        $baseAlias = Str::slug($base);

        if ($baseAlias === '') {
            $baseAlias = strtolower(Str::random(12));
        }

        $urlAlias = $baseAlias;
        $counter = 1;

        while (DB::table('vcards')->where('url_alias', $urlAlias)->exists()) {
