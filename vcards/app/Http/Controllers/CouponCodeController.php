...
class CouponCodeController extends AppBaseController
{
    ...
    public function applyCouponCode(Request $request, $couponCode = null)
    {
        ...
        $baseAmount = $newPlan['afterDiscount']['amountToPay'] ?? $newPlan['amountToPay'] ?? 0;

        if ($planTaxEnabled && $planTaxValue > 0) {
            $taxAmount = round(($baseAmount * $planTaxValue) / 100, 2);
            $totalWithTax = round($baseAmount + $taxAmount, 2);
        } else {
            $taxAmount = 0;
            $totalWithTax = round($baseAmount, 2);
        }

        $taxPreview = [
            'tax_percent'    => $planTaxValue,
            'tax_amount'     => $taxAmount,
            'total_with_tax' => $totalWithTax,
        ];

        $newPlan['tax_preview'] = $taxPreview;

        if (isset($newPlan['afterDiscount'])) {
            $newPlan['afterDiscount']['tax_preview'] = $taxPreview;
            $newPlan['afterDiscount']['amountToPay'] = $totalWithTax;
        }
