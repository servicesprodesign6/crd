...
class DashboardController extends AppBaseController
{
    ...
    public function index(): \Illuminate\View\View
    {
        ...
        if (\Request::is('sadmin/dashboard')) {
            $activeOrganisations = User::whereNull('organisation_id')
                ->where('is_organisation', true)
                ->where('is_active', User::IS_ACTIVE);

            $deActiveOrganisationIds = User::whereNull('organisation_id')
                ->where('is_organisation', true)
                ->where('is_active', User::DEACTIVATE)
                ->pluck('id')
                ->toArray();

            $activeOrganisationsCount = $activeOrganisations->count();
            $activeOrganisationIds = $activeOrganisations->pluck('id')->toArray();
            $deActiveOrganisationsCount = count($deActiveOrganisationIds);

            $activeOrganisationUsersCount = User::whereNotNull('organisation_id')
                ->where('is_organisation', false)
                ->where('is_active', User::IS_ACTIVE)
                ->whereIn('organisation_id', $activeOrganisationIds)
                ->count();

            $deActiveOrganisationUsersCount = User::whereNotNull('organisation_id')
                ->where('is_organisation', false)
                ->where(function ($query) use ($deActiveOrganisationIds) {
                    $query->where('is_active', User::DEACTIVATE)
                        ->orWhereIn('organisation_id', $deActiveOrganisationIds);
                })
                ->count();
            ...
            return view('dashboard.index', compact('activeUsersCount', 'deActiveUsersCount', 'activeVcard', 'deActiveVcard', 'activeOrganisationsCount', 'deActiveOrganisationsCount', 'activeOrganisationUsersCount', 'deActiveOrganisationUsersCount'));
        }

        $user = auth()->user();
        $activeVcardQuery = Vcard::whereTenantId($user->tenant_id)->whereStatus(1);
        $deActiveVcardQuery = Vcard::whereTenantId($user->tenant_id)->whereStatus(0);

        if ($user->organisation_id) {
            $activeVcardQuery->whereHas('assignedUser', function ($q) use ($user) {
                $q->where('users.id', $user->id);
            });
            $deActiveVcardQuery->whereHas('assignedUser', function ($q) use ($user) {
                $q->where('users.id', $user->id);
            });
        }

        $activeVcard = $activeVcardQuery->count();
        $deActiveVcard = $deActiveVcardQuery->count();

        $whatsappStoreQuery = WhatsappStore::whereTenantId(auth()->user()->tenant_id);
        if (isOrganisationUser()) {
            $whatsappStoreQuery->whereHas('assignedUser', function ($q) use ($user) {
                $q->where('users.id', $user->id);
            });
        }
        $storeIds = $whatsappStoreQuery->pluck('id');

        $totalWpTemplate = $whatsappStoreQuery->count();
        ...
        $vcardIdsQuery = Vcard::where('tenant_id', $user->tenant_id);
        if ($user->organisation_id) {
            $vcardIdsQuery->whereHas('assignedUser', function ($q) use ($user) {
                $q->where('users.id', $user->id);
            });
        }
        $vcardIds = $vcardIdsQuery->pluck('id')->toArray();
