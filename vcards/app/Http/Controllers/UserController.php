...
{
    private const IMPERSONATION_STACK_KEY = 'impersonation_stack';
    ...
    public function resendVerificationEmail(User $user): JsonResponse
    {
        if ($user->hasVerifiedEmail()) {
            return $this->sendError(__('messages.flash.user_email_verified'));
        }
    ...
    public function impersonate(User $user): RedirectResponse
    {
        ...
        if ($impersonateManager->isImpersonating()) {
            session()->push(self::IMPERSONATION_STACK_KEY, [
                'id' => $impersonateManager->getImpersonatorId(),
                'guard' => $impersonateManager->getImpersonatorGuardName(),
                'using' => $impersonateManager->getImpersonatorGuardUsingName(),
            ]);
        }
    ...
    {
        $impersonateManager = app('impersonate');
        $impersonator = $impersonateManager->getImpersonator();
        $impersonatedUser = getLogInUser();

        getLogInUser()->leaveImpersonation();

        $impersonationStack = session(self::IMPERSONATION_STACK_KEY, []);

        if (! empty($impersonationStack)) {
            $previousImpersonator = array_pop($impersonationStack);

            session()->put(self::IMPERSONATION_STACK_KEY, $impersonationStack);
            session()->put($impersonateManager->getSessionKey(), $previousImpersonator['id']);
            session()->put($impersonateManager->getSessionGuard(), $previousImpersonator['guard']);
            session()->put($impersonateManager->getSessionGuardUsing(), $previousImpersonator['using']);
        } else {
            session()->forget(self::IMPERSONATION_STACK_KEY);
        }

        if ($impersonator && $impersonator->hasRole('admin') && ($impersonator->is_organisation || $impersonator->organisation_id)) {
            return redirect(route('organisation.users.index'));
        }

        if ($impersonatedUser && $impersonatedUser->hasRole('admin') && $impersonatedUser->is_organisation) {
