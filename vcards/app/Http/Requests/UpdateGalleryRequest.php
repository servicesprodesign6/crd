...
class UpdateGalleryRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $hasMedia = $gallery->getMedia(Gallery::GALLERY_PATH)->isNotEmpty();

        if ($this->type == Gallery::TYPE_VIDEO) {
            $rules['video_file'] = [$hasMedia ? 'nullable' : 'required', 'file', 'mimes:mpeg,ogg,mp4,webm,3gp,mov,flv,avi,wmv,ts', 'max:100040',];
        }

        if ($this->type == Gallery::TYPE_AUDIO) {
            $rules['audio_file'] = [$hasMedia ? 'nullable' : 'required', 'file', 'mimes:mp3,wav,ogg', 'max:100040',];
        }
