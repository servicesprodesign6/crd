...
class ProductTransactionsTable extends LivewireTableComponent
{
    ...
    protected $listeners = ['refresh' => '$refresh','changeFilter','changeStatusFilter'];
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.common.status'), 'order_status')->view('product_transactions.column.status'),
    ...
    public function builder(): Builder
    {
        ...
        $user = getLogInUser();
        $query = ProductTransaction::whereHas('product.vcard', function($q) use ($tenantId, $user){
            $q->whereTenantId($tenantId);

            if (isOrganisationUser()) {
                $q->whereHas('assignedUser', function ($query) use ($user) {
                    $query->where('users.id', $user->id);
                });
            }
        });

        $query->when($paymentType !== null && $paymentType !== "", function ($q) use ($paymentType) {
            ...
            if ($paymentType == Product::RAZORPAY) {
              ...
              if ($paymentType == Product::IYZICO) {
                  $q->where('type', Product::IYZICO);
              }
        });

        $query->when($paymentStatus !== null && $paymentStatus !== "", function ($q) use ($paymentStatus) {
