...
class OrganisationTable extends LivewireTableComponent
{
    protected $model = User::class;
    public bool $showButtonOnHeader = true;
    public string $buttonComponent = 'organisation.add-button';
    ...
    public function configure(): void
    {
        ...
        $this->setPageName('organisation-table');
        $this->setDefaultSort('created_at', 'desc');
        $this->setSortingPillsStatus(false);
        $this->setQueryStringStatus(false);
        $this->resetPage('organisation-table');
    ...
    {
        return [
            Column::make(__('messages.organization.organization'), 'organisation_name')
                ->searchable()
                ->sortable()
                ->view('organisation.columns.name'),
            Column::make(
            ...
            Column::make(__('messages.common.action'), 'created_at')->view('organisation.columns.action'),
        ];
    }
    ...
    public function builder(): Builder
    {
        ...
        $query = User::role('admin')
            ->with(['media', 'subscriptions.plan', 'subscription.plan.planFeature'])
            ->where('users.id', '!=', getLogInUserId())
            ->whereNull('users.organisation_id')
            ->where('users.is_organisation', true);
    ...
    public function bulkDelete()
    {
        if (count($this->getSelected()) > 0) {
            $organisationIds = $this->getSelected();
            $this->dispatch('bulk-delete-organisation', $organisationIds);
        } else {
    ...
    public function deleteUser($userIds)
    {
        try {
            foreach ($userIds as $userId) {
                $user = User::where('is_organisation', true)->find($userId);
                if ($user) {
            ...
            $this->setBuilder($this->builder());
            $this->dispatch('delete-organisation-success');
            $this->clearSelected();
        } catch (\Exception $e) {
            Log::error('Bulk organisation deletion failed: '.$e->getMessage());
        }
    }
    ...
    public function resetPageTable($pageName = 'organisation-table')
    {
