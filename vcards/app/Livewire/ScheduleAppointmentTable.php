...
class ScheduleAppointmentTable extends LivewireTableComponent
{
            ...
            Column::make(__('messages.vcard.vcard_name'), 'vcard.name')
                ->sortable(function (Builder $query, $direction) {
                    return $query->orderBy(
                        Vcard::select('name')->whereColumn('vcards.id', 'schedule_appointments.vcard_id'),
                        $direction);
                })->searchable(),
    ...
    public function builder(): Builder
    {
        $type = $this->type;
        $status = $this->status;
        $vcardQuery = Vcard::whereTenantId(getLogInTenantId());
        $user = getLogInUser();
        if ($user->organisation_id) {
            $vcardQuery->whereHas('assignedUser', function ($q) use ($user) {
                $q->where('users.id', $user->id);
            });
        }
        $vcardIds = $vcardQuery->pluck('id')->toArray();
