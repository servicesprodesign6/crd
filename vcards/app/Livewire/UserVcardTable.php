...
class UserVcardTable extends LivewireTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.vcard.view_count'), 'id')
                ->view('vcards.columns.view_count'),
            Column::make(__('messages.vcard.status'), 'id')
                ->sortable()
                ->view('vcards.columns.status')
                ->hideIf(isOrganisationUser() && !getLogInUser()->organisationUserPermission->can_edit_vcard),
    ...
    {
        $query = Vcard::with(['tenant.user', 'template', 'analytics'])->where('tenant_id', getLogInTenantId());
        $user = getLogInUser();
