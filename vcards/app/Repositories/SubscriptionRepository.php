...
class SubscriptionRepository extends BaseRepository
{
    ...
    public function manageSubscription($planData): bool|array
    {
        ...
        $input = [
            ...
            'no_of_whatsapp_store' => $subscriptionPlan->no_of_whatsapp_store,
        ...
        // APPLY TAX AT THE VERY END
        $taxAmount = 0;

        if ($planTaxEnabled && $planTaxValue > 0 && $payableAmount > 0) {
            $taxAmount = round(($payableAmount * $planTaxValue) / 100, 2);
            $payableAmount = round($payableAmount + $taxAmount, 2);
        }

        $input['payable_amount'] = $payableAmount;

        $input['tax'] = $planTaxValue;
