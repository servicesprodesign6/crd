                ...
                <div class="d-flex align-items-center justify-content-between gap-3 mb-4"
                    ...
                    <div class="text-center">
                        <a class="vcard-seventeen-btn text-white d-block text-decoration-none"
                            href="{{ $vcardUrl }}" role="button">{{ __('messages.common.back') }}</a>
                    </div>
                </div>
                                ...
                                <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                    ...
                                    <div class="card product-card w-100 h-100 mb-3">
                                            ...
                                            <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                                ...
                                                <div
                                                    ...
                                                    @foreach ($product->media as $media)
                                                        <div>
                                                            <div class="product-img-height text-center w-100 mx-auto">
                                        ...
                                        <div class="card-body text-center pb-0">
                                            <h3 class="product-title text-white fw-5 text-center">{{ $product->name }}
                                            </h3>
                                            <p class="text-gray-100 mb-2">
                                                {{ $product->description }}</p>
                                            @if ($product->currency_id && $product->price)
                                                <span
                                                    class="fw-5 text-gradient amount text-center  product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, $product->currency->currency_code) }}</span>
                                            @elseif($product->price)
                                                <span
                                                    class="fw-5 text-gradient amount text-center  product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, getUserCurrencyIcon($vcard->user->id)) }}</span>
                                            @endif
                                </a>
                                            ...
                                            @if ($product->product_url)
                                                <div class="ms-1 mt-2">
                                                    <a href="{{ $product->product_url ?? 'javascript:void(0)' }}"
                                                        ...
                                                        class="btn send-btn btn-gradient btn-product-enquiry text-white @if(!$product->product_url) disabled @endif">
