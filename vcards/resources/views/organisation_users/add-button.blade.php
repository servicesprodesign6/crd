...
<div>
    <div class="dropdown d-flex align-items-center me-4 me-md-5" wire:ignore>
        <button class="btn btn btn-icon btn-primary text-white dropdown-toggle hide-arrow ps-2 pe-0" type="button"
            id="dropdownMenuOrganisationUserStatus" data-bs-auto-close="outside" data-bs-toggle="dropdown"
            aria-expanded="false">
            <i class='fas fa-filter'></i>
        </button>
        <div class="dropdown-menu py-0" aria-labelledby="dropdownMenuOrganisationUserStatus">
            ...
            <div class="p-5">
                ...
                <div class="mb-5">
                    ...
                    {{ Form::select('type', getTranslatedData($userStatusArr), null, ['class' => 'form-control form-select', 'data-control' => 'select2', 'id' => 'organisationUserStatus', 'wire:ignore']) }}
                </div>
                <div class="d-flex justify-content-end">
                    <button type="reset" id="organisationUserResetFilter" class="btn btn-secondary">{{ __('messages.common.reset') }}</button>
                </div>
            </div>
        </div>
    </div>
...
@if (canAddMoreOrganisationUsers())
    <div>
        <a type="button" class="btn btn-primary ms-auto" href="{{ route('organisation.users.create') }}">{{ __('messages.user.add_user') }}</a>
    </div>
@endif
