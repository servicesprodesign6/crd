                                    ...
                                    <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between mb-3">
                                        ...
                                        <label class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" value="{{ \App\Models\Plan::SSLCOMMERZ }}"
                                                name="payment_gateway[{{ \App\Models\Plan::SSLCOMMERZ }}]"
                                                {{ isset($selectedPaymentGateways['Sslcommerz']) ? 'checked' : '' }} id="sslcommerz_payment">
                                        </label>
                                    </div>
                                    <div class="sslcommerz-cred {{ !isset($selectedPaymentGateways['Sslcommerz']) ? 'd-none' : '' }}">
                                        <div class="mb-3">
                                            {{ Form::label('sslc_store_id', __('messages.setting.sslcommerz_store_id') . ':', ['class' => 'form-label mb-2 required fs-7']) }}
                                            <div class="position-relative">
                                                {{ Form::input(isset($setting['sslc_store_id']) ?? null ? 'password' : 'text', 'sslc_store_id', $setting['sslc_store_id'] ?? null, ['class' => 'form-control form-control-sm sslcommerz-key ' . (getLogInUser()->language == 'ar' || getLogInUser()->language == 'fa' ? 'payment-input-left' : 'payment-input-right'), 'placeholder' => __('messages.setting.sslcommerz_store_id'), 'data-toggle' => 'password']) }}
                                        ...
                                        <div class="mb-3">
                                            {{ Form::label('sslc_store_password', __('messages.setting.sslcommerz_store_password') . ':', ['class' => 'form-label sslcommerz-secret-label mb-2 required fs-7']) }}
                                            <div class="position-relative">
                                                {{ Form::input(isset($setting['sslc_store_password']) ?? null ? 'password' : 'text', 'sslc_store_password', $setting['sslc_store_password'] ?? null, ['class' => 'form-control form-control-sm sslcommerz-secret ' . (getLogInUser()->language == 'ar' || getLogInUser()->language == 'fa' ? 'payment-input-left' : 'payment-input-right'), 'placeholder' => __('messages.setting.sslcommerz_store_password'), 'data-toggle' => 'password']) }}
                                                <span class="position-absolute d-flex align-items-center top-0 bottom-0 payment-eye-icon @if (getLogInUser()->language == 'ar' || getLogInUser()->language == 'fa') start-0 pe-2 @else end-0 pe-2 ps-2 @endif input-icon input-password-hide cursor-pointer text-gray-600">
                                                    <i class="bi bi-eye-slash-fill fs-7"></i>
                                                </span>
                                            </div>
                                        ...
                                        <div class="mb-0">
                                            {{ Form::label('sslcommerz_mode', __('messages.setting.sslcommerz_mode') . ':', ['class' => 'form-label sslcommerz-mode mb-2 required fs-7']) }}
                                            {{ Form::select('sslcommerz_mode', $sslcommerzMode, $setting['sslcommerz_mode'] ?? null, ['class' => 'form-control form-control-sm sslcommerz-mode', 'data-control' => 'select2', 'data-minimum-results-for-search' => 'Infinity']) }}
                                        </div>
                                    </div>
                                </div>
                            </div>
