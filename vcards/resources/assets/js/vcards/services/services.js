...
listenClick(".service-delete-btn", function (event) {
    ...
    deleteItem(route("vcard.service.destroy", recordId), Lang.get("js.vcard_service"));
});
...
listenClick("#vcard-services-enquiry-button", function () {
    let id = $(this).attr("data-id");
    $.ajax({
        url: route("vcard.services.enquiry_button", id),
        type: "GET",
        success: function (result) {
            if (result.success) {
                displaySuccessMessage(result.message);
