...
let activeProductAiContext = null;

$(document).on('click', '.product-ai-description-btn', function (e) {
    e.preventDefault();
    activeProductAiContext = {
        formSelector: $(this).data("form"),
        targetSelector: $(this).data("target"),
    };
    $("#productAiDescriptionModal").modal("show");
});

listenHiddenBsModal("#productAiDescriptionModal", function () {
    $("#productAiDescriptionForm")[0].reset();
    $("#generateProductAiDescriptionBtn").prop("disabled", false);
    $("#productAiDescriptionCloseBtn").prop("disabled", false);
    $("#productAiDescriptionDiscardBtn").prop("disabled", false);
    $("#productAiDescriptionLoader").addClass("d-none");
    activeProductAiContext = null;

    const modalEl = document.getElementById("productAiDescriptionModal");
    if (modalEl) {
...
listenSubmit("#productAiDescriptionForm", function (e) {
    e.preventDefault();

    const prompt = $("#productAiDescriptionTextarea").val().trim();
    if (!prompt) {
        displayErrorMessage(Lang.get("js.please_enter_prompt"));
        return false;
    }

    if (!activeProductAiContext) {
        return false;
    }

    const form = $(activeProductAiContext.formSelector);
    const target = $(activeProductAiContext.targetSelector);
    const name = form.find('input[name="name"]').val();
    const price = form.find('input[name="price"]').val();
    const productUrl = form.find('input[name="product_url"]').val();

    $("#generateProductAiDescriptionBtn").prop("disabled", true);
    $("#productAiDescriptionCloseBtn").prop("disabled", true);
    $("#productAiDescriptionDiscardBtn").prop("disabled", true);
    $("#productAiDescriptionLoader").removeClass("d-none");
    ...
    if (modalEl) {
        ...
        $("#productAiDescriptionModal").modal({ backdrop: 'static', keyboard: false });
    }

    $.ajax({
        url: route("vcard.products.generate.ai.description"),
        type: "POST",
        data: {
            prompt: prompt,
            name: name,
        ...
        success: function (result) {
            ...
            if (result.success) {
                target.val(result.description);
                $("#productAiDescriptionModal").modal("hide");
                displaySuccessMessage(
                    Lang.get("js.description_generated_successfully")
                );
            } else {
